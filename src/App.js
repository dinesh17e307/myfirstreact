import './App.css';
import React,{useState} from 'react'
import Person from './Components/Person'
const App =props=>{
  const [personState,setpersonState]=useState(
    {
        person:[{nam:'dinesh',place:'sathy'},
        {nam:'ganesh',place:'chennai'},
        {nam:'kalai',place:'erode'}]
      });

const change=()=>{
    setpersonState({
        person:[
            {nam:'dheena',place:'sathy'},
            {nam:'dilip',place:'chennai'},
            {nam:'kuna',place:'erode'}
          ]
    })
}
  return (
    <div className="App">
     <h1>hi welcome ,this is my first app going to develop</h1>
     <button onClick={change}>Change or visit state</button>
     <Person names={personState.person[0].nam} place={personState.person[0].place}/>
     <Person names={personState.person[1].nam} place={personState.person[1].place}>my hobby:creating websites</Person>
     <Person names={personState.person[2].nam} place={personState.person[2].place}/>
    </div>
    //React.createElement('div',{className:App},React.createElement('h1',null,'this is how our normal html gets compiled'));
);
  
}

export default App;
